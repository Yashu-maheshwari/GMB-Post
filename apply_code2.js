...
// 6. JSON Parse logic FIXED
const parseTarget = `        if (parsed.summary && parsed.summary.trim().length > 20) {\r
          return {\r
            success: true,\r
            summary: parsed.summary.trim(),\r
            topic_title: parsed.topic_title || selectedPillar.name,\r
            pillar_id: selectedPillar.id,\r
            cta_url: config.ctaUrl,\r
            model_used: model\r
          };\r
        }`;
const parseTargetFallback = `        if (parsed.summary && parsed.summary.trim().length > 20) {
          return {
            success: true,
...
if (code.includes(parseTarget)) {
    code = code.replace(parseTarget, newParseBlock);
} else if (code.includes(parseTargetFallback)) {
    code = code.replace(parseTargetFallback, newParseBlock);
} else {
    console.log("Could not find exact parse block");
}
